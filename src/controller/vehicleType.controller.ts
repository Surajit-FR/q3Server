...
export const addVehicleType = asyncHandler(async (req: CustomRequest, res: Response) => {
    ...
    if (existinfVehicleType) {
        return handleResponse(res, "error", 400, "Vehicle type with the same name already exists.")
    };
    ...
    if (!newVehicleType) {
        return handleResponse(res, "error", 500, "Something went wrong while adding the Shift.")
    };

    return handleResponse(res, "success", 201, newVehicleType, "Vehicle type added Successfully")
})

// fetchVehicleTypebyId controller
export const fetchVehicleTypebyId = asyncHandler(async (req: Request, res: Response) => {
    ...
    const responseData = results.length
        ? handleResponse(res, "success", 200, results[0], "Vehicle type retrieved successfully.")
...
export const fetchVehicleTypes = asyncHandler(async (req: CustomRequest, res: Response) => {
    const results = await vehicleTypeModel.aggregate([
        { $match: { isDeleted: false } },

    ]);
    if (results.length) {
        return handleResponse(res, "success", 200, results, "Vehicle types retrieved successfully.");
    } else {
