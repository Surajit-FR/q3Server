...
export const sendOTP = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!phoneNumber) {
        return res.status(400).json({ success: false, message: "phoneNumber are required" });
    }
...
export const verifyOTP = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!phoneNumber || !otp || !purpose) {
        return handleResponse(res, "error", 400, "phoneNumber, otp, and purpose are required")
    }
    ...
    switch (purpose) {
        ...
        case "forgetPassword": {
            return handleResponse(res, "success", 200, "", "OTP Verified Successfully")
        }
        case "endJob":
            return handleResponse(res, "success", 200, "", "OTP Verified Successfully")
        case "verifyPhone":
