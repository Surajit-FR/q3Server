    ...
    io.on("connection", (socket) => {
        ...
        socket.on("chatMessage", async (message) => {
            ...
            try {
                ...
                await (0, chat_controller_1.saveChatMessage)({
                    fromUserId: userId,
                    toUserId,
                    content,
                    isRead,
                ...
                await (0, chat_controller_1.updateChatList)(userId, toUserId, content, now);
                await (0, chat_controller_1.updateChatList)(toUserId, userId, content, now);
                if (recipientSocketId) {
        ...
        socket.on("markMessagesRead", async ({ toUserId }) => {
            try {
                await chat_model_1.default.updateMany({ toUserId, isRead: false }, { $set: { isRead: true } });
        ...
        socket.on("acceptServiceRequest", async (requestId) => {
            ...
            const customerId = await (0, towingServiceBooking_controller_1.fetchAssociatedCustomer)(requestId);
        ...
        socket.on("declineServiceRequest", async (requestId) => {
